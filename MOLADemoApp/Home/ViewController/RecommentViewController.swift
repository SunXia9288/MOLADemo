...
extension RecommentViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.model = recGameList[indexPath.row]
        return cell
    }

    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc = PagingViewController()
        vc.gameModel = recGameList[indexPath.row].app_summary
        vc.gameModel?.banner = recGameList[indexPath.row].image
